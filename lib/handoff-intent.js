const HANDOFF_REASONS = {
  ...
  reclamo: {
    ...
    patterns: [
      /\breclamo\b/i,
      /\bqueja\b/i,
      /problema con (?:el |la |un |una )?(?:pedido|compra|entrega|env[ií]o|producto|servicio|atenci[oó]n|bot|respuesta)/i,
      /\bmala atenci[oó]n\b/i,
      /\berror\b/i,
      /no lleg[oó]/i,
      /no funciona/i
    ]
...
function detectHandoffIntent(texto, options = {}) {
  const text = String(texto || "").trim();
  if (!text) return null;

  const ignoredReasons = new Set(options.ignoredReasons || []);
  const allowedReasons = options.allowedReasons ? new Set(options.allowedReasons) : null;

  for (const [reason, config] of Object.entries(HANDOFF_REASONS)) {
    if (ignoredReasons.has(reason) || (allowedReasons && !allowedReasons.has(reason))) continue;
