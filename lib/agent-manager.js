...
const { HANDOFF_REASONS, detectHandoffIntent, isHumanHandoffEnabled, isLLMResponseUncertain, cleanLLMResponse } = require("./handoff-intent");
...
REGLA CRITICA: Si no tenes informacion suficiente para responder con certeza, empeza tu respuesta con "NO Sé RESPONDER:" seguido de una breve razon. No inventes datos ni informacion que no este en el contexto proporcionado.
...
class AgentManager {
  constructor(agentConfig, deps = {}) {
    ...
    this.lastBotReplyAt = 0;
    this.replyDelayMs = this.resolveReplyDelayMs(agentConfig);
  ...
  resolveReplyDelayMs(agentConfig) {
    const features = agentConfig.features || {};
    const configured = features.replyDelayMs ?? process.env.BOT_REPLY_DELAY_MS;
    if (configured === undefined || configured === null || configured === "") return DEFAULT_REPLY_DELAY_MS;

    const delay = Number(configured);
    return Number.isFinite(delay) && delay > 0 ? delay : DEFAULT_REPLY_DELAY_MS;
  }

  async waitBeforeBotReply() {
    if (!this.replyDelayMs || this.replyDelayMs <= 0 || !this.lastBotReplyAt) return;

    const elapsed = Date.now() - this.lastBotReplyAt;
    const waitMs = this.replyDelayMs - elapsed;
    if (waitMs > 0) {
  ...
  async responderBot(message, texto) {
    await this.waitBeforeBotReply();
    const respuesta = await message.reply(texto);
    this.lastBotReplyAt = Date.now();
  ...
  getHandoffDetectionOptions(flowType = null) {
    if (!flowType) return {};
    return { ignoredReasons: ["precio"] };
  }

  async tryRejectUnsafeFlowMessage(message, userId, texto) {
    ...
    if (tipoAtaque) {
      ...
      this.statsManager.registrarError(userId, "hijacking").catch(error => {
        log(`[${this.id}] ⚠️ Error registrando error: ${error.message}`, "WARN");
      });
    ...
    if (texto.length > MAX_MESSAGE_LENGTH) {
      this.statsManager.registrarError(userId, "mensaje_largo").catch(error => {
        log(`[${this.id}] ⚠️ Error registrando error: ${error.message}`, "WARN");
      });
      await this.responderBot(message, `Tu mensaje es muy largo (${texto.length} caracteres). Para esta demo, enviá un mensaje de máximo ${MAX_MESSAGE_LENGTH} caracteres.`);
      return true;
    }
  ...
  async tryStartHandoff(message, userId, texto, detectionOptions = {}) {
    if (!isHumanHandoffEnabled(this.config)) return false;

    const intent = detectHandoffIntent(texto, detectionOptions);
  ...
  async triggerAutoHandoff(message, userId, reason, detail) {
    const intent = { reason, label: detail || HANDOFF_REASONS[reason]?.label || reason };
    await this.startHumanHandoff(message, userId, intent);
  }
  ...
  async handleIncomingMessage(message) {
    ...
    const flowType = this.config.features && this.config.features.flowType;
    if (flowType && await this.tryRejectUnsafeFlowMessage(message, userId, texto)) return;

    if (await this.tryStartHandoff(message, userId, texto, this.getHandoffDetectionOptions(flowType))) return;

    log(`[${this.id}] ✅ Procesando mensaje de ${userId}: "${texto}"`);
    if (flowType) {
      try {
        const flow = require(`./flows/${flowType}-flow.js`);
        await flow.handleMessage(this, message, userId, texto);
      } catch (error) {
        log(`[${this.id}] ❌ Error en flow ${flowType}: ${error.message}`, "ERROR");
        await this.responderBot(message, "Disculpá, hubo un error. Intentá de nuevo.");
      }
      return;
    }
  ...
  async handleDemoFlow(message, userId, texto) {
    ...
    if (this.estadosUsuario[userId] === ESTADOS.DEMO_ESPERANDO_CONSULTA) {
      ...
      this.statsManager.registrarRespuestaIA(userId).catch(error => {
        log(`[${this.id}] ⚠️ Error registrando respuesta IA: ${error.message}`, "WARN");
      });
...
Respondé solo sobre capacidades de la plataforma y cerrá la demo.`;
      ...
      const { uncertain, detail } = cleanLLMResponse(respuesta);
      if (uncertain) {
        await this.triggerAutoHandoff(message, userId, "no_sabe", "El agente no supo responder");
        return;
      }

      await this.responderBot(message, respuesta);
    } catch (error) {
      this.statsManager.registrarError(userId, "llm_fallo").catch(err => {
        log(`[${this.id}] ⚠️ Error registrando error: ${err.message}`, "WARN");
      });
      log(`[${this.id}] ⚠️ Error generando respuesta demo: ${error.message}`, "WARN");
      if (isHumanHandoffEnabled(this.config)) {
        await this.triggerAutoHandoff(message, userId, "no_sabe", "Error del asistente virtual");
        return;
      }
  ...
  async handlePedidoFlow(message, texto, userId) {
    ...
    if (temaProhibido) {
      this.statsManager.registrarError(userId, "prohibido").catch(error => {
        log(`[${this.id}] ⚠️ Error registrando error: ${error.message}`, "WARN");
      });
    ...
    if (insulto) {
      ...
      if (isHumanHandoffEnabled(this.config)) {
        await this.triggerAutoHandoff(message, userId, "reclamo", "Lenguaje ofensivo detectado");
        return;
      }
    }
