...
function createConversationState(dataDir) {
  ...
  function pauseUser(userId, reason, metadata = {}) {
  ...
  function listChats() {
    ...
    for (const [userId, mensajes] of Object.entries(history)) {
      ...
      chats.push({
        ...
        handoffReason: pauseEntry.handoffReason || null,
        handoffReasonLabel: pauseEntry.handoffReasonLabel || null,
        handoffRequestedAt: pauseEntry.handoffRequestedAt || null
      });
    }
  ...
  function clearAllData() {
    ...
    fs.writeFileSync(pausasPath, JSON.stringify({ global: false, usuarios: {} }, null, 2));
    return { historial: true, pausas: true };
  }
