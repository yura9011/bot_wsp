...
async function handleMessage(agent, message, userId, texto) {
  ...
  if (estado === ESTADOS.PEDIDO) {
    if (texto === "0" || /menu|menú|ver menú|volver/i.test(texto)) {
      agent.estadosUsuario[userId] = ESTADOS.MENU;
      await agent.responderBot(message, getMenu());
      return;
    }
    ...
    if (contextoProductos) {
      mensajeConContexto = `Sos el asistente de una rotisería. Respondé directamente al cliente, en segunda persona (vos), como si le hablaras por WhatsApp. NUNCA hables en tercera persona sobre el cliente.

Menú disponible:\n${contextoProductos}\n\n---\n\nConsulta del cliente: ${texto}\n\nReglas:\n1. Respondé como si fueras el dueño de la rotisería, con buena onda\n2. Usá "vos" y hablá directo al cliente\n3. Si pide algo que no está en el menú, ofrecé alternativas similares\n4. Si el monto es alto (más de $15000), sugerí que un operador lo ayude\n5. Sé breve y conversacional`;
    } else {
      mensajeConContexto = `Sos el asistente de una rotisería. Respondé directamente al cliente, en segunda persona (vos). NUNCA hables en tercera persona sobre el cliente.

Consulta del cliente: ${texto}\n\nNo se encontraron productos exactos. Interpretá qué quiere el cliente y ofrecé opciones del menú. Respondé como si fueras el dueño, con buena onda. Sé breve.`;
    }
    ...
    try {
      if (!agent.model) {
        agent.model = agent.inicializarModelo ? agent.inicializarModelo() : null;
      }
