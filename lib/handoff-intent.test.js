...
test("can ignore selected handoff reasons by context", () => {
  assert.equal(detectHandoffIntent("cuanto cuesta una milanesa", { ignoredReasons: ["precio"] }), null);
  assert.deepEqual(detectHandoffIntent("quiero hablar con una persona", { ignoredReasons: ["precio"] }), {
    reason: "quiere_humano",
    label: "Quiere hablar con una persona"
  });
});

test("does not treat normal pet problem as a complaint", () => {
  assert.equal(detectHandoffIntent("tengo un problema con mi perro"), null);
  assert.deepEqual(detectHandoffIntent("tengo un problema con el pedido"), {
    reason: "reclamo",
    label: "Reclamo"
  });
});
